...
static bool s_te_wait_pending;
static bool s_te_timed_out;

static void IRAM_ATTR display_te_isr(void *arg)
{
    BaseType_t wake = pdFALSE;
    xSemaphoreGiveFromISR((SemaphoreHandle_t)arg, &wake);
    if (wake) {
        portYIELD_FROM_ISR();
    }
}

static void display_refresh_start(lv_event_t *event)
{
    (void)event;
    s_te_wait_pending = true;
}

static esp_err_t display_draw_partial(lv_display_t *display, esp_lcd_panel_handle_t panel,
                                      int x_start, int y_start, int x_end, int y_end,
                                      const void *pixels, void *user_data)
{
    (void)display;
    (void)user_data;
    if (s_te_wait_pending) {
        s_te_wait_pending = false;
        /* Discard idle-time edges; all areas in this refresh share one fresh TE edge. */
        (void)xSemaphoreTake(s_te_sem, 0);
        const bool ready = xSemaphoreTake(s_te_sem, pdMS_TO_TICKS(25)) == pdTRUE;
        if (!ready && !s_te_timed_out) {
            ESP_LOGW(TAG, "TE wait timed out (25 ms); continuing partial refresh without sync");
        } else if (ready && s_te_timed_out) {
            ESP_LOGI(TAG, "TE synchronization recovered");
        }
        s_te_timed_out = !ready;
    }
    /* The adapter's DMA completion callback owns LVGL's flush-ready notification. */
    return esp_lcd_panel_draw_bitmap(panel, x_start, y_start, x_end, y_end, pixels);
}

static void display_te_delete(void)
{
    if (s_te_sem) {
        gpio_isr_handler_remove(BSP_LCD_TE);
        vSemaphoreDelete(s_te_sem);
        s_te_sem = NULL;
    }
}

static esp_err_t display_te_init(void)
{
    s_te_sem = xSemaphoreCreateBinary();
    ESP_RETURN_ON_FALSE(s_te_sem, ESP_ERR_NO_MEM, TAG, "allocate TE semaphore failed");
    s_te_wait_pending = true;
    s_te_timed_out = false;
    /* CO5300 mode 1 drives TE high during vertical blanking (datasheet section 5.3). */
    const gpio_config_t config = {
        .pin_bit_mask = 1ULL << BSP_LCD_TE,
    ...
    if (ret == ESP_OK) {
        ret = gpio_isr_handler_add(BSP_LCD_TE, display_te_isr, s_te_sem);
    }
    if (ret != ESP_OK) {
        vSemaphoreDelete(s_te_sem);
        s_te_sem = NULL;
...
static void round_qspi_area(lv_area_t *area, void *user_data)
{
    ...
    /* CO5300 RASET requires an even start row and an even row count. */
    const int max_y = BSP_LCD_V_RES - 1;
    int y1 = area->y1 < 0 ? 0 : (area->y1 > max_y ? max_y : area->y1);
    int y2 = area->y2 < 0 ? 0 : (area->y2 > max_y ? max_y : area->y2);
    area->y1 = (y1 / 2) * 2;
    y2 = ((y2 + 2) / 2) * 2 - 1;
    area->y2 = y2 > max_y ? max_y : y2;
}
...
#if CONFIG_BSP_CO5300_ENABLE_TE
    const bool te_enabled = active.tear_avoid_mode == ESP_LV_ADAPTER_TEAR_AVOID_MODE_TE_SYNC;
    if (te_enabled) {
        /* TE_SYNC forces full rendering in the adapter; the BSP synchronizes partial flushes. */
        display_config.profile.require_double_buffer = false;
        active.tear_avoid_mode = ESP_LV_ADAPTER_TEAR_AVOID_MODE_NONE;
    }
#else
    ...
    if (te_enabled) {
        ...
        if (ret == ESP_OK) {
            const esp_lv_adapter_draw_bitmap_callbacks_t callbacks = {
                .custom_draw_bitmap = display_draw_partial,
            };
            ret = esp_lv_adapter_set_draw_bitmap_callbacks(s_display, &callbacks, NULL);
        }
        if (ret == ESP_OK) {
            lv_display_add_event_cb(s_display, display_refresh_start, LV_EVENT_REFR_START, NULL);
        }
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "partial-refresh TE setup failed (%s)", esp_err_to_name(ret));
            esp_lv_adapter_unregister_display(s_display);
            s_display = NULL;
            display_te_delete();
            return NULL;
        }
        ESP_LOGI(TAG, "partial refresh: PSRAM single buffer, TE GPIO%d once per refresh", BSP_LCD_TE);
    }
#endif
